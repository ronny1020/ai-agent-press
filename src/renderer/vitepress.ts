...
export async function prepareTemporaryDirectory(
    ...
    isAllMode
      ...
      : nodes.filter(
          (n) =>
            n.type === 'skill' ||
            n.type === 'instruction' ||
            n === pickIndexNode(nodes),
        )
  )

  const { sidebar, nodeToLink, repoSidebarItems, globalSidebarItems } = computeSidebar(activeNodes, { isAllMode })
  ...
  for (const node of activeNodes) {
    try {
      ...
      if (isMarkdown) {
        ...
        const codeLanguage = EXTENSION_LANGUAGE_MAP[extension] ?? extension
        finalBody = `\n\n\`\`\`${codeLanguage}\n${content}\n\`\`\`\n\n`
      }
  ...
  // Write config as JS (not plain JSON) so we can include functions like shikiSetup
  const config = `export default {
  ...${JSON.stringify(vitepressConfig, undefined, 2)},
  markdown: {
    lineNumbers: true,
    async shikiSetup(shiki) {
      // Suppress "language not loaded, falling back to txt" warnings for custom/unknown languages
      const origWarn = console.warn
      console.warn = (...args) => {
        if (typeof args[0] === 'string' && args[0].includes('is not loaded')) return
